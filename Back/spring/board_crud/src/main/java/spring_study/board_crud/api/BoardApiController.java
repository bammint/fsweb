...
public class BoardApiController {
    ...
    public WrapperClass boarder_list(){
        List<Board> boardList = boardService.findBoards();
        List<BoardDto> boardDtoList = boardList.stream().map(b -> new BoardDto(b))
    ...
    public ResponseEntity update_board(@RequestBody BoardDto boardDto){
        ...
        HttpHeaders headers = new HttpHeaders();
        Map<String, String> body = new HashMap<>();
        HttpStatus status = HttpStatus.NO_CONTENT; // 204 잘 수정되었음을 의미
        try{
    ...
    public ResponseEntity delete_board(@RequestBody BoardDeleteDto boardDeleteDto){
        ...
        HttpHeaders headers = new HttpHeaders();
        Map<String, String> body = new HashMap<>();
        HttpStatus status = HttpStatus.NO_CONTENT; // 204 잘 생성되었음을 의미
        try{
