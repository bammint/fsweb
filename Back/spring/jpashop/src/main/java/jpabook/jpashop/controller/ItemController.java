...
// 병합(merge)
// 준영속 엔티티란?
// 영속성 엔티티가 더이상 관리하지 않는 엔티티이며 이미 한번
// parsist되어서 식별자가 있는 경우를 뜻한다
// 값이 변경되어도 알아서 반영이 되지 않기 때문에 따로 값을 반영해야한다

// 파라미터 item 객체는 준영속 엔티티이며 em,merge를 통해
// Item merge는 파라미터로 받은 item으로 값들이 다 교체된다

// merge 할 경우 강제적으로 모든 필드의 값을 교체하고 값이 없으면
// null로 돌아가기 때문에 조심해야한다

// merge 할 경우 강제적으로 모든 필드의 값들이 변경되기 때문에
// "엔티티를 변경할 때는 항상 변경감지를 사용한다"

// @Controller에서 식별자(primary key)만 넘기고 핵심 비즈니스
// 로직은 @Transactional(@Service)안에서 처리하는 것이 좋다